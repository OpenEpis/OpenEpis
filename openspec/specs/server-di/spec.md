...
The server SHALL create a `Container` instance at startup, register `PostgresStorageService` as the `IStorageService` implementation, and pass the container to all route plugins. Route plugins SHALL resolve `IStorageService` from the container, never importing a concrete implementation directly.
